...
## Phase 2: vLLM Adapter Development (TDD)
- [ ] Task: Create `metachat_core/tests/test_vllm_model.py` and write failing tests for `VLLMModel` initialization and health check.
- [ ] Task: Implement `VLLMModel.__init__` with health check in `metachat_core/core/models/vllm.py`.
- [ ] Task: Write failing tests for `VLLMModel.generate` (supporting stop sequences and error handling).
- [ ] Task: Implement `VLLMModel.generate` using the OpenAI client.
- [ ] Task: Write failing tests for `VLLMModel.count_tokens`.
- [ ] Task: Implement `VLLMModel.count_tokens` with fallback logic.
- [ ] Task: Conductor - User Manual Verification 'Phase 2: vLLM Adapter Development (TDD)' (Protocol in workflow.md)
